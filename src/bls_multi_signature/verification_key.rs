...
impl BlsVerificationKey {
    ...
    pub fn add(&self, other: &BlsVerificationKey) -> BlsVerificationKey {
    ...
    pub fn aggregate_unsafe(vks: &[BlsVerificationKey]) -> BlsVerificationKey {
        assert_ne!(vks.len(), 0);
        ...
        let mut verification_keys: Vec<blst_p2> = Vec::with_capacity(vks.len());
        for vk in vks.iter() {
            verification_keys.push(vk_from_p2_affine(vk));
        }

        let init = verification_keys.pop().unwrap();
        let res = verification_keys
            .iter()
            .fold(init, |acc, sig| p2_add(&acc, &sig));

        let aggr_sig: BlstVk = p2_affine_to_vk(&res);

        BlsVerificationKey(aggr_sig)
    }
